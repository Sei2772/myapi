...
app.post('/add', async function (req, res) {
    ...
    if (!orderData.product_name || !orderData.product_weight || !orderData.product_price) {
        return res.status(400).send({
            error: true,
            message: 'Please provide product_name, product_weight, and product_price'
        });
    }

    try {
        // Step 1: ดึง idProduct จากชื่อสินค้า
        const resultProduct = await query('SELECT idProduct FROM product WHERE ProductName = ?', [orderData.product_name]);

        if (resultProduct.length === 0) {
            return res.status(404).send({ error: true, message: 'ไม่พบสินค้าในระบบ' });
        }

        const idProduct = resultProduct[0].idProduct;

        // Step 2: ดึง Order ID ล่าสุด
        const resultOrders = await query('SELECT id FROM orders ORDER BY id DESC LIMIT 1');
        let orderId;

        if (resultOrders.length === 0) {
            // ยังไม่มี order => สร้างใหม่
            const resultInsertOrder = await query('INSERT INTO orders (created_at) VALUES (NOW())');
            orderId = resultInsertOrder.insertId;
        } else {
            orderId = resultOrders[0].id;
        }

        // Step 3: ตรวจสอบว่าสินค้านี้มีอยู่แล้วใน orderdetail หรือไม่
        const existingDetail = await query('SELECT * FROM orderdetail WHERE order_id = ? AND idProduct = ?', [orderId, idProduct]);

        if (existingDetail.length > 0) {
            // มีสินค้าแล้ว -> อัปเดตน้ำหนัก
            await query(
                'UPDATE orderdetail SET product_weight = ? WHERE order_id = ? AND idProduct = ?',
                [orderData.product_weight, orderId, idProduct]
            );
        } else {
            ...
            await query('INSERT INTO orderdetail SET ?', [newOrderDetail]);
        }

        // Step 4: ตัด stock
        await query('UPDATE product SET quantity = quantity - ? WHERE idProduct = ?', [orderData.product_weight, idProduct]);

        return res.send({ success: true, message: 'ดำเนินการเพิ่มหรืออัปเดตรายการเรียบร้อยแล้ว และตัด stock แล้ว' });
